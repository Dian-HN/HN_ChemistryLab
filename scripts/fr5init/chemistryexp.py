...
class HNchemistry(fr5robot):
    ...
    def F101_pour_01(self,pour_position,pour_direction,sel_num,is_force_sensor = False,is_display = False):
        '''
        指定位置倾倒指定仪器
        ...
        is_display:是否为演示，如果是，机械臂会自动复位
        '''
        print("目标物体绝对xy坐标:",pour_position)
        ...
        if pour_direction == "yn":
            print("倾倒方向是：y轴负方向")
            # 数据处理y
            pour_position[1] = pour_position[1] + 150
    ...
    def Auto_Weight(self,fr5_B):
        ...
        # 机械臂B抓取称量后的量筒
        string = ' '.join(map(str, liangtong_xy_output))
        fr5_B.F101_catch_02(string, "xp" , "3" , False, is_display = False)
        fr5_B.MoveL(0.0, 0.0, 300.0)
        
        # 机械臂移动到水浴锅中三颈烧瓶上方，执行倾倒 TODO
        string = ' '.join(map(str, sanjing_xy_output))
        fr5_B.F101_pour_01(string, "xp" , "3" , False, is_display = False)
        
        # 机械臂B将倾倒后的量筒放置在指定位置 TODO
        ...
        # 机械臂B抓取称量后的烧杯
        string = ' '.join(map(str, shaobei_xy_output))
        fr5_B.F101_catch_02(string, "xp" , "1" , False, is_display = False)
        fr5_B.MoveL(0.0, 0.0, 300.0)
        
        # 机械臂移动到水浴锅中三颈烧瓶上方，执行倾倒 TODO
        string = ' '.join(map(str, sanjing_xy_output))
        fr5_B.F101_pour_01(string, "xp" , "1" , False, is_display = False)
        
        # 机械臂B将倾倒后的量筒放置在指定位置 TODO
        string = ' '.join(map(str, shaobei_xy_used))
        fr5_B.F101_put_01(string, "xp" , "1" , False, is_display = False)
        fr5_B.Go_to_start_zone()
